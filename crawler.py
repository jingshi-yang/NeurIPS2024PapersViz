...
url = 'https://nips.cc/Conferences/2024/Schedule?type=Poster'


def get_author_info(author):
    author_url = f'https://nips.cc/Conferences/2024/Schedule?showSpeaker={author}'
...
def paper_retriever(paper_index):
    paper = {}
    paper_url = f'https://nips.cc/Conferences/2024/Schedule?showEvent={paper_index}'
    while True:
        try:
...
def main():
    ...
    with open('nips2024.txt', 'w') as f:
        f.write(text)

    with open('nips2024.txt', 'r') as f:
        text = f.read()
    ...
    # write to json file
    with open('nips2024.json', 'w') as f:
        for paper in papers:
